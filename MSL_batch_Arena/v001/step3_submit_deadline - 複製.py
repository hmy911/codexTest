...
Usage (commandline): mayapy step3_submit_deadline.py --shotcodes 101_0280_0060 101_0280_0080
...
# ══════════════════════════════════════════════
SHOTCODES    = ["101_0280_0060", "101_0280_0080"]
...
def submit_layer(layer, shotcode, seq, settings, auto_scene,
    ...
    scene_name   = Path(auto_scene).stem              # ← 加這行
    ...
    output_root = (PROJECT_ROOT
        / f"shots/{seq}/{shotcode}/images/3d_renders/lighting/v002")
    output_root.mkdir(parents=True, exist_ok=True)

    layer_short = layer.replace("rs_", "")
    # Maya 會自建：v002/{scene_name}/{layer}/{layer}.####.exr
    output_dir  = output_root / scene_name / layer_short
    ...
    job_info = {
        ...
        "OutputDirectory0"          : str(output_dir).replace("/", "\\"),
    ...
    plugin_info = {
        ...
        # OutputFilePath = output_dir（明確指定，不讓 Maya 自建子目錄）
        "OutputFilePath"          : str(output_dir).replace("/", "\\"),
        # OutputFilePrefix 留空，避免 Maya 自動建 <Scene>/<Layer>/ 子目錄
        "OutputFilePrefix"        : "",
        "RenderLayer"             : layer,
        "UsingRenderLayers"       : "1",
