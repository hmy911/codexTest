...
def check_camera_animated(cam_transform):
    ...
    # 只有一個 keyframe（任一 channel）
    if len(all_keys) == 1:
        return False, "single_keyframe"

    # 所有 keyframe 值完全一樣（靜態）
...
def process_shot(shotcode):
    ...
    full_dag = cmds.ls(cam_transform, long=True)[0]
    print(f"  DAG path: {full_dag}")
    try:
